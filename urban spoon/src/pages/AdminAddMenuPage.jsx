      ...
      <main className="mx-auto grid min-h-[calc(100vh-5.5rem)] max-w-[1440px] grid-cols-[280px_1fr] max-[1100px]:grid-cols-1">
        ...
        <section className="px-6 py-6 max-[700px]:px-4">
          ...
          <div className="rounded-[1.1rem] border border-[#e4e7ee] bg-white p-6">
            <h2 className="text-[1.6rem] font-bold text-[#11182f]">Add New Menu Item</h2>
            <p className="mt-1 text-[0.95rem] text-[#5a6377]">Create a new dish for your restaurant menu.</p>
            ...
            <form className="mt-5 grid gap-4" onSubmit={handleSubmit}>
              ...
              <div className="grid grid-cols-2 gap-4 max-[820px]:grid-cols-1">
                <label className="grid gap-1.5">
                  ...
                  <input name="name" value={formData.name} onChange={handleChange} className="h-11 rounded-lg border border-[#dbe1ec] px-3 text-[0.95rem] outline-none focus:border-[#ef2c5b]" placeholder="e.g. Truffle Pasta" />
                </label>

                <label className="grid gap-1.5">
                  <span className="text-[0.86rem] font-semibold text-[#2e374c]">Category *</span>
                  <input name="category" value={formData.category} onChange={handleChange} className="h-11 rounded-lg border border-[#dbe1ec] px-3 text-[0.95rem] outline-none focus:border-[#ef2c5b]" placeholder="e.g. Main Course" />
                </label>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 max-[820px]:grid-cols-1">
                <label className="grid gap-1.5">
                  ...
                  <input type="number" min="0" step="0.01" name="price" value={formData.price} onChange={handleChange} className="h-11 rounded-lg border border-[#dbe1ec] px-3 text-[0.95rem] outline-none focus:border-[#ef2c5b]" placeholder="e.g. 250" />
                </label>

                <label className="grid gap-1.5">
                  <span className="text-[0.86rem] font-semibold text-[#2e374c]">Image URL</span>
                  <input name="imageUrl" value={formData.imageUrl} onChange={handleChange} className="h-11 rounded-lg border border-[#dbe1ec] px-3 text-[0.95rem] outline-none focus:border-[#ef2c5b]" placeholder="https://..." />
                </label>
              </div>

              <label className="grid gap-1.5">
                <span className="text-[0.86rem] font-semibold text-[#2e374c]">Description *</span>
                <textarea name="description" value={formData.description} onChange={handleChange} rows={4} className="rounded-lg border border-[#dbe1ec] px-3 py-2 text-[0.95rem] outline-none focus:border-[#ef2c5b]" placeholder="Short description of the item" />
              </label>
